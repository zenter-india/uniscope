-- Follow-up to 20260909120000_extract_location_from_university_names:
-- that pass handled ", <district>" (comma-separated). Many rows instead
-- have the district stuck on the end with just a space —
-- "DISTRICT HOSPITAL MALE BUDAUN", "Government Medical College Satna". Same
-- treatment: when the trailing word(s) validate as a real district of that
-- college's state AND the name still reads as an institution afterwards,
-- move the district to universities.city (when empty) and strip it.
--
-- Medical stream only. Engineering/Law/Dental already have a district in
-- universities.city for ~100% of rows (so their card is already correct),
-- and their name formats are far more varied — the strip mangled a real
-- chunk of them in testing ("University of Lucknow" -> "University of",
-- "... Regional Campus - Madurai" -> "... -"). Medical names are dominated
-- by the clean "Government Medical College <district>" pattern.
--
-- Guards against over-stripping:
--   * word-boundary match ("...MALE BUDAUN", not "...MALEBUDAUN")
--   * the remaining name must still contain an institution keyword
--   * remaining name length >= 6
--   * the remaining name must not end on a connector / dangling punctuation
-- Non-district trailing words are left untouched. End-anchored + guarded,
...
SET "city" = COALESCE(NULLIF(btrim(u."city"), ''), g."district"),
    "name" = btrim(regexp_replace(u."name", '\s+' || regexp_replace(g."district", '([().\[\]{}*+?^$|\\-])', '\\\1', 'g') || '\s*$', '', 'i')),
    "updated_at" = now()
FROM gaz g
WHERE g."state" = u."state"
  AND u."is_active"
  AND u."stream" = 'Medical'
  AND u."name" !~ ','                                            -- comma rows already done
  AND u."name" ~* ('(^|\s)' || regexp_replace(g."district", '([().\[\]{}*+?^$|\\-])', '\\\1', 'g') || '\s*$')
  AND btrim(regexp_replace(u."name", '\s+' || regexp_replace(g."district", '([().\[\]{}*+?^$|\\-])', '\\\1', 'g') || '\s*$', '', 'i')) ~* '(college|hospital|institute|university|vidyalaya|vidyapeeth|sciences|centre|center|academy|faculty|foundation|school|society|mission|trust|polytechnic|research)'
  AND length(btrim(regexp_replace(u."name", '\s+' || regexp_replace(g."district", '([().\[\]{}*+?^$|\\-])', '\\\1', 'g') || '\s*$', '', 'i'))) >= 6
  AND btrim(regexp_replace(u."name", '\s+' || regexp_replace(g."district", '([().\[\]{}*+?^$|\\-])', '\\\1', 'g') || '\s*$', '', 'i')) !~* '(\s(of|the|and|for|dist|distt|tal|taluka|near|at|po|ps|via|dt)\.?|[-–:,])\s*$';
