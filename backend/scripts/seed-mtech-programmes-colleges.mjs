/**
 * Seed M.Tech/M.E colleges + their per-college specializations (Discipline
 * column) from an AISHE programme-level export (see scripts/data/README.md
 * for provenance). Same upgrade as seed-btech-programmes-colleges.mjs did
 * for B.Tech/B.E — replaces the flat PG_ENGINEERING_SPECIALIZATIONS-driven
 * M.Tech/M.E field with the same curated University+Program pattern
 * Medical's MD/MS/DNB/Diploma/DM-MCh, Dental's MDS, and now B.Tech use.
 *
 ...
 * 'Engineering' or unset, so this script can never repeat the
 * Law-seed-scripts bug of pushing a level onto a different stream's row.
 *
 * PUNCTUATION-INSENSITIVE MATCHING (see scripts/data/README.md's
 * "punctuation-insensitive matching" section — found while seeding
 * B.Tech Programmes): matching key uses normalizeForMatch(), not a plain
 * name.toLowerCase().trim(), so a college spelled with different
 * punctuation/spacing between this AISHE source and the already-seeded
 * pg-engineering-colleges.json (NBA source) is still correctly matched
 * rather than creating a near-duplicate row. The JSON generation script
 * groups colleges using this exact same normalization, for the same
 * "generation key must match the seed key" reason documented for
 * btech-programmes-colleges.json.
 *
 * Reuses University rows already created by seed-pg-engineering-colleges.mjs
 * (same college, matched by name+state+district) — pushes "PG" onto
 * `levels` if missing. Creates an 'M.TECH' Program row per college
 * (uppercase — see the casing-bug note in scripts/data/README.md),
 * specializations from the source, description = district (same
 * convention as DNB/DM-MCh/Diploma/MDS/B.TECH).
 *
 ...
 *
 * Usage:
 *   node scripts/seed-mtech-programmes-colleges.mjs            # write
 *   node scripts/seed-mtech-programmes-colleges.mjs --dry-run   # report only
 */
...
async function main() {
  const colleges = JSON.parse(
    await readFile(path.join(DATA, 'mtech-programmes-colleges.json'), 'utf8'),
  );
  ...
  console.log(
    `Universities: ${stats.matched} matched existing, ${stats.created} created, ` +
      ...
      `M.TECH programs: ${stats.programsCreated} created, ${stats.programsUpdated} updated.` +
