/**
 * Seed M.Tech/M.E (Engineering PG) colleges (see scripts/data/README.md
 * for provenance). Like seed-btech-colleges.mjs, this source has no
 ...
 *
 * Matching/creation rules mirror seed-btech-colleges.mjs (which in turn
 * mirrors seed-dnb-colleges.mjs): finds an existing University by
 * name+state match (case-insensitive), with pre-existing rows tracked
 * via a "claimed" set so a repeated name+state key can't steal a row
 * another entry already claimed. Where available, district goes on
 * `University.city` — 6 colleges in the raw source had no state/district
 * at all and were dropped at generation time rather than guessed at
 * (see scripts/data/README.md).
 *
 * Many of these colleges also offer B.Tech and will already exist as
 * University rows from the btech seed — matching correctly pushes
 * "PG" onto their `levels` (already `["UG"]`) rather than duplicating,
 * same as how seed-mds-colleges.mjs reuses BDS-seeded rows.
 *
 ...
 *
 * Usage:
 *   node scripts/seed-pg-engineering-colleges.mjs            # write
 *   node scripts/seed-pg-engineering-colleges.mjs --dry-run   # report only
 */
...
async function main() {
  const colleges = JSON.parse(
    await readFile(path.join(DATA, 'pg-engineering-colleges.json'), 'utf8'),
  );
