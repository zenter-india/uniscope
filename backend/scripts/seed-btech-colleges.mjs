/**
 * Seed B.Tech/B.E (Engineering UG) colleges (see scripts/data/README.md
 * for provenance). Like seed-bds-colleges.mjs, this source has no
 ...
 * already claimed. Where available, district goes on `University.city`
 * (same convention as seed-bds-colleges.mjs) — 28 colleges in the raw
 * source had no state/district at all and were dropped at generation
 * time rather than guessed at (see scripts/data/README.md).
 *
 ...
 *
 * Usage:
 *   node scripts/seed-btech-colleges.mjs            # write
 *   node scripts/seed-btech-colleges.mjs --dry-run   # report only
 */
...
async function main() {
  const colleges = JSON.parse(await readFile(path.join(DATA, 'btech-colleges.json'), 'utf8'));
