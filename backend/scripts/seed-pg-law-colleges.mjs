/**
 * Seed PG Law (LL.M. / M.L.) colleges (see scripts/data/README.md for
 * provenance). Like seed-law-ug-colleges.mjs, this source has no
 ...
 *
 * Matching/creation rules mirror seed-pg-engineering-colleges.mjs:
 * finds an existing University by name+state match (case-insensitive),
 * with pre-existing rows tracked via a "claimed" set so a repeated
 * name+state key can't steal a row another entry already claimed.
 * Where available, district goes on `University.city`.
 *
 * Many of these colleges also offer UG Law and will already exist as
 ...
 *
 * Usage:
 *   node scripts/seed-pg-law-colleges.mjs            # write
 *   node scripts/seed-pg-law-colleges.mjs --dry-run   # report only
 */
...
async function main() {
  const colleges = JSON.parse(await readFile(path.join(DATA, 'pg-law-colleges.json'), 'utf8'));
